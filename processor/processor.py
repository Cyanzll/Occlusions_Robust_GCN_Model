...
class Processor(IO):
    ...
    def load_data(self):
        ...
        if self.arg.phase == 'train':
            ...
            # 导入完整的骨架数据集 - 注意此处的shuffle参数！
            self.data_loader['complete_skeleton'] = torch.utils.data.DataLoader(
                dataset=Feeder(**self.arg.complete_skeleton_args),
                batch_size=self.arg.batch_size,
                shuffle=False,
                num_workers=self.arg.num_worker * torchlight.ngpu(
                    self.arg.device),
    ...
    def start(self):
        ...
        if self.arg.phase == 'train':
            for epoch in range(self.arg.start_epoch, self.arg.num_epoch):
                ...
                self.train()
                self.writer.add_scalar("Mean_loss/Epoch", self.epoch_info['mean_loss'], epoch)
                ...
                # 测试代码：train后直接保存
                if (epoch + 1) % 10 == 0 or (epoch + 1) == self.arg.num_epoch:
                    filename = 'epoch{}_model.pt'.format(epoch + 1)
                    self.io.save_model(self.model, filename)
    ...
    def get_parser(add_help=False):
        ...
        parser.add_argument('--complete_skeleton_args', action=DictAction, default=dict(), help='the arguments of complete skeletons')
